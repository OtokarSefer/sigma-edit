...
rl.question("Sisestage oma nimi: ", Nimi => {
  rl.question("Sisestage lubatud kiirus: ", lubatudKiirus => {
    rl.question("Sisestage tegelik kiirus: ", tegelikKiirus => {
      let trahv = 0;
      if (Number(tegelikKiirus) < Number(lubatudKiirus)) {console.log(Nimi + " sõitis lubatud kiirusega ega saa trahvi :)");}
      else {trahv = (Number(tegelikKiirus) - Number(lubatudKiirus)) * 3;
        if (trahv > 190) {trahv = 190;}
